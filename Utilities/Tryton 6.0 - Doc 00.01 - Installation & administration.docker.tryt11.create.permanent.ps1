...
Write-Host "---------------------------------"
docker run --env DB_HOSTNAME=tryt11-postgres --env DB_PASSWORD=${POSTGRES_PASSWORD} --network tryt11-network --interactive --tty --rm tryton/tryton:6.0 trytond-admin -d tryt11 --all # 'tryton/tryton:6.0 trytond-admin -d tryt11' : from 'tryton/tryton:6.0' run 'trytond-admin' against the 'tryt11' database
docker exec -tiu postgres tryt11-postgres psql -c '\l+'
...
docker run --name tryt11 --env DB_HOSTNAME=tryt11-postgres --env DB_PASSWORD=${POSTGRES_PASSWORD} --volume ${TRYTON_VOL_FI}:/var/lib/trytond/db --network tryt11-network -p 8011:8000 --detach tryton/tryton:6.0
docker run --name tryt11-cron --env DB_HOSTNAME=tryt11-postgres --env DB_PASSWORD=${POSTGRES_PASSWORD} --volume ${TRYTON_VOL_FI}:/var/lib/trytond/db --network tryt11-network --detach tryton/tryton:6.0 trytond-cron -d tryt11 # 'tryton/tryton:6.0 trytond-cron -d tryt11' : from 'tryton/tryton:6.0' run 'trytond-cron' against the 'tryt11' database
