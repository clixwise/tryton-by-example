...
# 8011
# 5443:5432
# Obtain latest version of tryt11
#
...
Write-Host "---------------------------------"
docker run --link tryt11-postgres:postgres -e DB_PASSWORD=Password --rm -it tryton/tryton:6.0 trytond-admin -d tryt11 --all # Define database tables
...
Write-Host "-------------------------------------------------------------"
docker run --name tryt11 -p 8011:8000 --link tryt11-postgres:postgres -e DB_PASSWORD=Password -d tryton/tryton:6.0 # Start a tryt11 instance
